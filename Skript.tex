...
Dieses Skript umfasst die Themen des geplanten Musiktheoriekurses auf der ersten
Hälfte der CdE-WinterAkademie 2025\,·\,26, in dem die theoretischen Grundlagen
mehrstimmiger klassischer Musik besprochen werden.  Die Konzepte werden zumeist
am Beispiel vierstimmiger Chorsätze erläutert, sind aber leicht auf andere
Satzformen übertragbar.
...
machen.  Die Frequenz eines Tones wird in Hz (Schwingungen pro Sekunde)
angegeben – je größer die Frequenz ist, desto höher klingt der Ton für
...
Die erste Tonbezeichnung im Sinne der Aufgabenstellung ist folgende: Einen Ton
...
Stimmgabeln erzeugen genau diesen Ton.
...
\emph{Oktave}:\footnote{Woher der Bezug zur Zahl 8 kommt, klärt sich in \cref{subsec:int}.} er
entspricht dem Frequenzverhältnis 1\,:\,2.  Der Ton eine Oktave über dem
Kammerton hat folglich eine Frequenz von 880\,Hz.  Das menschliche Ohr nimmt
Töne, die sich nur um Oktaven unterscheiden, als sehr ähnlich wahr; dieses
Phänomen nennt sich \emph{Oktavidentität}.
...
Wir widmen uns nun dem zweiten Bestandteil des Tonnamens, nämlich der zuvor
erwähnten \emph{Tonklasse}, die Oktaven außer Acht lässt.  Dabei beschränken wir
uns zunächst auf die weißen Tasten: Wir fixieren einen beliebigen Oktavraum und
weisen den darin befindlichen weißen Tasten die in \cref{fig:weiNam}
dargestellten Namen (\rb c, \rb d, \rb e, \rb f, \rb g, \rb a, \rb h) zu.  Es sei nochmal darauf
hingewiesen, dass diese Namen den Ton nur \emph{bis auf Oktaven} bestimmen:
...
Tonklasse ein \rb{is} an den Buchstaben angehängt, während bei einer Tiefalteration
ein \rb{es} (bei Vokalen nur ein \rb{s}) angehängt wird. Eine Ausnahme gibt es: Die
Tiefalteration von h heißt \rb{b} – aus Gründen™. Der Oktavraum wird in der Benennung
vom Ausgangston »geerbt«. So wird beispielsweise die hochalterierte Form von c’
als cis’ bezeichnet.\looseness-1
...
den jeweiligen Ausgangstönen geerbt wird.  So ist das Grundintervall zwischen
...
\item Primen, Quarten, Quinten und Oktaven klingen
  nur in einer Ausprägungsform plausibel und kommen in der Musik auch
...
\item Sekunden, Terzen, Sexten und Septimen klingen in genau zwei
  Ausprägungsformen plausibel, die auch in etwa gleich häufig vorkommen. Diese
  beiden Ausprägungsformen heißen \emph{groß} und \emph{klein}, wobei die kleine
  Variante stets einen Halbtonschritt weniger hat als die große. Die großen
  Varianten umfassen exakt 2\,\acr{HTS}, 4\,\acr{HTS}, 9\,\acr{HTS} und
  11\,\acr{HTS}.  So ist beispielsweise c’\,:\,e’ eine große Terz, aber
  d’\,:\,f’ eine kleine Terz.
...
ist das \emph{Komplementärintervall} der k7 (und umgekehrt).

\subsection{Größere Intervalle}

Um ein Intervall wie c’\,:\,des” zu benennen, geht man wie folgt vor: Wir zählen
...
ein klar erkennbares »Zentrum« haben, d.\,h. einen Ton, der über weite Strecken
...
Die Kombination aus Grundton und Tongeschlecht (also Dur oder Moll) konstituiert
...
erhalten (z.\,B. ein f, das ein Auflösungszeichen braucht).\looseness-1
...
Mozarts Sonate nicht schon nach 7 Takten zuende ist!
...
»aufgerollt« wird (auch wenn es musikalisch nicht ganz adäquat ist). Dieser
Kreis heißt \emph{Quintenzirkel} und ist ein Teil von \cref{fig:QZ}
...
und dis als Generalvorzeichen mitnehmen.

\begin{figure}[h]
  \centering
  \input{fig/quintenzirkel}
  \caption{Dur- und Moll-Tonarten, angeordnet im Quintenzirkel. Auf 6 Uhr haben
    wir enharmonisch »verklebt«, auch wenn es ein bisschen
    wehtut.}\label{fig:QZ}
\end{figure}
...
\begin{figure}[h]
  ...
  \caption{Dreiklänge, die aus den Tönen einer C-Dur-Skala bestehen. }\label{fig:stufenDur}
\end{figure}
...
\item Der Durakkord \textsc{i} heißt \emph{Tonika} (T) und bildet das »Zentrum«
  des Stücks, zu dem man spätestens am Ende zurückkehren möchte.
...
Obige Diskussion ist kein Spezifikum vom C-Dur, sondern funktioniert in jeder
...
über einen besonderen Ton sprechen.
...
starke Strebetendenz aufwärts zum Grundton (»man hat den Gipfel schon fast
erreicht«) und ist daher ein wichtiges Mittel, um auf ein Ende hinzuarbeiten.

Die Dominante einer Durtonart strebt nicht zuletzt deswegen
zurück zur Tonika, weil sie den Leitton als Terz enthält.

\subsection{Stufen und Funktionen in Moll}
\label{subsec:SFMoll}

Wir wollen nun das Schema aus \cref{subsec:SFDur} auf Moll übertragen. Soll
dabei die Dominante wie in Dur zur Tonika streben, müssen wir den Leitton
künstlich in Stufe \textsc{v} einbauen, indem wir die Terz dieses Akkords
hochalterieren. Dadurch wird Stufe \textsc{v} vom Moll- zum Durakkord. Es
ergeben sich die in \cref{fig:SMoll} dargestellten Dreiklänge.
...
\begin{figure}[h]
  ...
  \caption{Die sieben Stufen in Moll.}\label{fig:SMoll}
\end{figure}
...
schrittweise von unten erreichen, ohne dabei einen Hiatus zu erzeugen, so muss man
zwangsläufig auch noch den 6. Ton hochalterieren.
...
\begin{enumerate}
\item Läuft man von unten aus auf den Grundton zu, so will zum Grundton
...
\item Läuft man vom Grundton aus nach unten, so nutzt man skaleneigene Töne (so
  wie im vierten Takt zu sehen).
\end{enumerate}
...
\begin{figure}[h]
  ...
  \caption{Die Takte 143–148 aus dem Eingangschor der 1. Kantate des
    Weihnachtsoratoriums von Bach (\acr{BWV\,248}). Das Stück ist eigentlich in
...
erhebt, ein »Schritt« zu sein). Ein Beispiel findet sich in \cref{fig:singet}.
...
also als \emph{Tonikagegenklang}, bezeichnet.
...
\item Endet eine Kadenz nicht wie erwartet in der Tonika, spricht man von einem
  ...
  – Grundtton« hören, weswegen der »Ersatz« für die Tonika den Grundton
  enthalten sollte. Daher sind die üblichen Formen des Trugschlusses D\,–\,Tp in
  Dur und D\,–\,tG in Moll.
...
Diese Beschreibungen genügen, um die versprochenen »Faustregeln« für die
...
\item Es sollte sich nur in reinen, kleinen und großen Intervallen bewegt
  werden. Eine Ausnahme stellt die v7 wie in \cref{fig:singet} dar.
\end{itemize}
...
die Bereich, in denen sie gut singen können) finden sich in
...
\item Alle Stimmen sind in derselben Tonart geschrieben. Gleichzeitig
  ...
  \item Grundton und Terz dürfen doppelt vorkommen, die Quinte nicht.
  \item Im Bass darf der Grundton oder die Terz stehen; die Quinte aber nur in
    Ausnahmefällen. Was davon der Fall ist, wird als Index an die Funktion
...
\item Bei jedem Wechsel sollten wenigstens zwei der Bewegungsrichtungen
  (Steigen, Fallen, Liegen) vorkommen. Zu vermeiden ist, dass sich alle
  Stimmen in dieselbe Richtung bewegen (\emph{Satzrutsch}).\looseness-1
...
\end{itemize}
Dasselbe gilt für Oktavparallelen, mit einer Ausnahme: Oktavantiparallelen, bei
denen sich die Stimmen (wie im vierten Beispiel von \cref{fig:QPb}) in
verschiedene Richtungen bewegen, sind üblich.\looseness-1
\begin{figure}[h]
  \centering
  \includegraphics{ly/s-parallelen1}\vspace*{-10px}
...
leider kein besseres Argument als blanke Empirie anbieten.
...
In jeder der in \cref{subsec:Kadenzen} beschriebenen Kadenzen (selbst bei einem
Trugschluss) enthält der vorletzte Akkord den Leitton und der letzte Akkord den
Grundton. Diejenige Einzelstimme, die den Leitton hat, möchte dann gerne auch
den Grundton singen. Deshalb gilt die Stimmführungsregel: Der Leitton muss
aufwärts in den Grundtton geführt werden.

Würde man also in einer Kadenz den Leitton verdoppeln (was nach den bisherigen
Regeln legitim wäre; er ist ja die Terz der Dominante), würden zwangsläufig
Oktavparallelen entstehen.

Von obiger Regel gibt es eine Ausnahme (die jedoch keine Leittonverdopplung
rechtfertigt), unter der vor allem Mittelstimmen regelmäßig leiden: Der Grundton
kann \emph{in gleicher Lage} von einer anderen Stimme über\-nommen werden, siehe
\cref{fig:lat}. Dies heißt \emph{latente Auflösung}.
...
erreicht, dass die Stimmen rhythmisch voneinander unabhängig werden; dies ist
der erste Schritt in Richtung \emph{Polyphonie}. Trotzdem solte man beim
...
\item Tauchen in mehreren Stimmen gleichzeitig Zwischennoten auf, so müssen sie
  zueinander »konsonant geführt« werden, d.\,h. sie müssen (bis auf Oktaven)
  eine Terz oder Sexte bilden.
\item Durch Zwischennoten können Quint- und Oktavparallelen entstehen, die nicht
  leicht auffallen.  Hier sollte man besonders aufpassen.
\end{itemize}
...
\emph{Quart-}, \emph{Sext-} und \emph{Nonenvorhalte}. In \cref{fig:Vorhalt}
...
\item %\textbf{Konsonante Einführung der Vorhaltsnote}\\
  Vorhalte sollten \emph{konsonant eingeführt} werden: Die Vorhaltsnote muss
  regulärer Bestandteil des zuvor erklingenden Akkords sein und dann bis zum
  Vorhalt liegen bleiben. Oft wird diese Note
  übergebunden, wodurch die Stimmen rhythmisch versetzt sind, siehe
  \cref{fig:Vorhalt}.\looseness-1
...
\begin{itemize}
\item Bei der Auflösung der Dominante muss die Septime schrittweise abwärts
  geführt werden (also in die Terz der Tonika oder die Quinte des
  Trugschlusses). Wie beim Leitton ist auch eine latente Auflösung der Septime
  möglich, dennoch darf sie nicht verdoppelt werden.
\item Terz (Leitton) und Septime müssen in diesem Akkord also genau einmal
  vorkommen. Für die zwei übrigen Stimmen bleiben zwei Möglichkeiten: Grundton
  und Quinte, oder zweimal Grundton.
\item Im Bass darf der Grundton, die Terz und sogar die Septime liegen.  Liegt
  ...
  im Bass, siehe Beispiel 3 von \cref{fig:d7}.
\end{itemize}
...
$\muS^{\kern.4px{}^6_5}$ bzw.\ $\mus^{\kern.4px{}^6_5}$.  Üblich ist auch der
französische Name \textit{sixte ajoutée} (»hinzugefügte Sexte«).

Da die Sexte der Subdominante der Quinte der Dominante entspricht, wird die
Umsetzung der Akkordfolge S\,–\,D erleichtert.  In Moll hat der
Quintsextakkord einen Tritonus zwischen der k3 und der g6.  Einige
Regeln:
\begin{itemize}
\item Wie schon erwähnt darf die Quinte darf nicht weggelassen werden
  (andernfalls läge im Falle von Dur stattdessen die Sp vor).
\item Im Bass darf der Grundton oder die Terz liegen, in Moll auch die
  ...
  mit der Sp (mit hinzugefügter 7) auszuschließen.
\item Die Quinte und die Sexte des Akkords sollten in unterschiedliche
  Richtungen weitergeführt werden (oder eine von ihnen bleibt liegen).
\end{itemize}
...
\begin{figure}[h]
  ...
  \caption{Einige Erscheinungsformen des Subdominant-Quintsextakkords.}
\end{figure}

\section{Erweiterung des harmonischen Rahmens}

Um einen Satz harmonisch reichhaltiger zu gestalten, können auch Funktionen
außerhalb des harmonischen Rahmens verwendet werden. Dabei nutzen wir die
Bewegung D\,–\,T/t relativ zu verschiedenen Grundfunktionen.  Wir beginnen
mit der üblichsten Erscheinungsform: der Doppeldominante.
...
\item Zwischendominantsept-Akkorde zur Subdominante sind reizvoll: In Dur ist
  ihre Septime skalenfremd, in Moll ihre Terz.\vspace*{-5pt}
\end{itemize}
...
anderen Tonart (hier: d-Moll) aufzuhalten.  In der Barockzeit geschieht dies
noch nur übergangsweise. Bleibt man jedoch in der neuen Tonart, so hat man den
...
zum Grundtton und die Septime der Dominante zur Terz der Tonika strebt, strebt
nun die k9 zur Quinte der Tonika, die eine k2 unter ihr liegt. Dadurch wird die
Dominante sogar noch spannungsreicher. Einige Regeln:
\begin{itemize}
...
werden. Dadurch entsteht ein Fünfklang, den wir \emph{Dominantseptnonakkord}
nennen. Bei ihm darf die Quinte oder der Grundton fehlen. Entsprechend
notieren wir ihn als $\smash{\muD^{\kern.3px{}^9_7}}$ oder \smash{\mbox{\raisebox{-.2px}{\rotatebox{40}{\rule{4mm}{.7px}}}\hspace*{-8.4px}$\smash{\muD^{\kern.3px{}^9_7}}$}}.

Wieder kann jeder der Töne im Bass liegen. Die None und die Septime müssen
schritt\-weise abwärts geführt werden, der Leitton aufwärts.\vspace*{-3px}
...
\begin{figure}[h]
  ...
  \caption{G.\,F.\,Händel, \emph{Tochter Zion}, Siegeschor aus \emph{Judas
      Maccabäus} (\acr{HWV}\,63), mit Gegenbewegungen zwischen Sopran und
    Bass.}\label{fig:gegen}\vspace*{-8px}
\end{figure}
...
Wenn die Kette hinreichend lang weitergeführt wird, aber dabei nicht moduliert werden
soll, muss zwangsläufig irgendwann eine Quinte vermindert bzw. eine Quarte übermäßig
sein.
\begin{figure}[h]
  \centering
  \includegraphics{ly/s-quintfall}
  \caption{Ein Quintfall in: A.\,Vivaldi, \emph{Herbst}, Satz 1 in F-Dur aus
    \emph{Le quattro stagioni} (op.\,8, \acr{RV}\,269).}\vspace*{-8px}
\end{figure}
